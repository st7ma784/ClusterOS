...
# Create ClusterOS config directory (consistent with image builds)
sudo mkdir -p /etc/clusteros

# Try to propagate existing Tailscale config from current node first
if [ -z "$TAILSCALE_OAUTH_CLIENT_ID" ] && [ -z "$TAILSCALE_OAUTH_CLIENT_SECRET" ] && [ -z "$TAILSCALE_AUTHKEY" ]; then
    if [ -f /etc/clusteros/tailscale.env ]; then
        echo ">>> Using existing Tailscale configuration from current node"
        sudo cp /etc/clusteros/tailscale.env /tmp/tailscale.env.template
        echo ">>> Tailscale config will be copied to new node"
    fi
fi

# Create tailscale.env file in the same format as the image build
if [ -f /tmp/tailscale.env.template ]; then
    echo ">>> Using Tailscale configuration from current node"
    sudo cp /tmp/tailscale.env.template /etc/clusteros/tailscale.env
    sudo chmod 600 /etc/clusteros/tailscale.env
    echo ">>> Tailscale credentials copied from current node"
elif [ -n "$TAILSCALE_OAUTH_CLIENT_ID" ] || [ -n "$TAILSCALE_OAUTH_CLIENT_SECRET" ] || [ -n "$TAILSCALE_AUTHKEY" ]; then
    sudo tee /etc/clusteros/tailscale.env > /dev/null <<TSENV
# Tailscale Configuration for ClusterOS
# Generated by cluster-os-install
...
TSENV
    sudo chmod 600 /etc/clusteros/tailscale.env
    echo ">>> Tailscale credentials configured in /etc/clusteros/tailscale.env"
else
    echo ">>> No Tailscale credentials provided - manual configuration required"
    echo ">>> Edit /etc/clusteros/tailscale.env and run: sudo systemctl enable --now tailscale-auth.service"
fi
