...
if ! id clusteros &>/dev/null; then
    ...
    ok "clusteros user created (password locked, key auth only)"
else
...
if [ -f "$SCRIPT_DIR/authorized_keys" ]; then
    mkdir -p "$CLUSTEROS_HOME/.ssh"
    cat "$SCRIPT_DIR/authorized_keys" >> "$CLUSTEROS_HOME/.ssh/authorized_keys"
    sort -u "$CLUSTEROS_HOME/.ssh/authorized_keys" -o "$CLUSTEROS_HOME/.ssh/authorized_keys"
    chmod 700 "$CLUSTEROS_HOME/.ssh"
    chmod 600 "$CLUSTEROS_HOME/.ssh/authorized_keys"
    chown -R clusteros:clusteros "$CLUSTEROS_HOME/.ssh"
    ok "SSH authorized key installed ($(wc -l < "$CLUSTEROS_HOME/.ssh/authorized_keys") key(s))"
else
...
# ── WiFi always-on (bridge gateway operation) ─────────────────────────────────────
# When a laptop running ClusterOS is plugged into the cluster switch, Ubuntu/NM may
# disable WiFi or let the driver enter power-save mode, killing Tailscale connectivity.
# ClusterOS needs WiFi (Tailscale mesh) and Ethernet (LAN cluster) simultaneously —
# the node acts as a bridge so the wired cluster can reach the wider Tailscale network.
#
# Fixes applied:
#   1. Disable WiFi power-save at the driver level so the NIC stays associated.
#   2. Write an NM conf.d snippet that sets wifi.powersave=2 (off) for all connections.
#   3. Ensure the cluster WiFi SSID (TALKTALK665317) auto-connects and does NOT claim
#      the IPv4 default route when ethernet is present (so ethernet handles LAN traffic
#      and WiFi stays connected for Tailscale without routing conflicts).
#   4. Reload NetworkManager non-disruptively to pick up the new config.

_WIFI_SSID="${CLUSTER_WIFI_SSID:-TALKTALK665317}"

# Driver-level: disable power-save on every wireless interface present.
for _wlan_dir in /sys/class/net/*/wireless; do
    [ -d "$_wlan_dir" ] || continue
    _wlan_iface=$(basename "$(dirname "$_wlan_dir")")
    if iw dev "$_wlan_iface" set power_save off 2>/dev/null; then
        ok "WiFi power-save disabled on $_wlan_iface"
    fi
done

# NM config: keep wifi.powersave=2 (disabled) for all connection profiles.
mkdir -p /etc/NetworkManager/conf.d
cat > /etc/NetworkManager/conf.d/99-clusteros-wifi.conf << 'NMEOF'
[connection]
# Keep WiFi active alongside Ethernet for Tailscale bridge operation.
wifi.powersave=2
NMEOF

# Connection profile: ensure cluster SSID auto-connects and defers default route
# to Ethernet so both interfaces coexist without routing conflicts.
if command -v nmcli &>/dev/null && nmcli connection show "$_WIFI_SSID" &>/dev/null 2>&1; then
    nmcli connection modify "$_WIFI_SSID" \
        connection.autoconnect yes \
        ipv4.never-default true \
        ipv6.never-default true 2>/dev/null && \
        ok "NM: '$_WIFI_SSID' set to autoconnect, never-default (Ethernet owns default route)" || \
        warn "Could not modify NM connection '$_WIFI_SSID' — manual check may be needed"
fi

# Non-disruptive reload: picks up conf.d changes without dropping active connections.
