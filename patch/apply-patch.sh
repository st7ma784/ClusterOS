...
# Only call `systemctl stop` when the service is already RUNNING (active).
# If we are inside node-agent's own ExecStartPre the unit state is "activating"
# and systemctl stop sends SIGTERM back to this very script — killing us before
# .bootstrapped is written and causing an infinite restart / boot loop.
_na_state=$(systemctl show node-agent --property=ActiveState --value 2>/dev/null || echo inactive)
if [ "$_na_state" = "active" ]; then
...
if ! id clusteros &>/dev/null; then
    ...
    ok "clusteros user created"
else
    ok "clusteros user already exists"
fi

# ── Install authorized SSH key ───────────────────────────────────────────────
# The patch bundle includes authorized_keys generated by `make patch`.
# Append (not overwrite) so re-runs add the key without removing existing ones.
CLUSTEROS_HOME=$(getent passwd clusteros | cut -d: -f6)
SSH_KEY_INSTALLED=false
if [ -f "$SCRIPT_DIR/authorized_keys" ]; then
    # Use || true on every write: a read-only /home (ProtectHome in old service
    # units) must not abort the script before the Tailscale auth section runs.
    mkdir -p "$CLUSTEROS_HOME/.ssh" 2>/dev/null || true
    if [ -d "$CLUSTEROS_HOME/.ssh" ]; then
        cat "$SCRIPT_DIR/authorized_keys" >> "$CLUSTEROS_HOME/.ssh/authorized_keys" 2>/dev/null || true
        sort -u "$CLUSTEROS_HOME/.ssh/authorized_keys" -o "$CLUSTEROS_HOME/.ssh/authorized_keys" 2>/dev/null || true
        chmod 700 "$CLUSTEROS_HOME/.ssh" 2>/dev/null || true
        chmod 600 "$CLUSTEROS_HOME/.ssh/authorized_keys" 2>/dev/null || true
        chown -R clusteros:clusteros "$CLUSTEROS_HOME/.ssh" 2>/dev/null || true
        ok "SSH authorized key installed"
        SSH_KEY_INSTALLED=true
    else
        warn "Could not create $CLUSTEROS_HOME/.ssh (read-only fs?) — SSH key not installed this run"
    fi
else
    warn "patch/authorized_keys not found — SSH key auth not configured (run: make patch)"
fi
...
# ── Lock the default account password ──────────────────────────────────────────
# images/ubuntu/cloud-init/user-data sets the well-known "clusteros" password
# (lock_passwd: false) so console/SSH access works before this script runs on
# the very first boot. Lock the account password now that an SSH key is in
# place — this also covers cloud-init-provisioned nodes, where the user
# already exists above and was previously never locked, leaving the default
# password valid for local/console logins indefinitely. Skip if no SSH key
# was installed, to avoid a full lockout (no password and no key).
if [ "$SSH_KEY_INSTALLED" = true ]; then
    passwd -l clusteros 2>/dev/null || true
    ok "Default account password locked (key auth only)"
else
    warn "Skipping password lock — no SSH key installed (would cause full lockout)"
fi
...
if [ -f "$SCRIPT_DIR/cloudflare.env" ]; then
    mkdir -p /etc/clusteros
    install -m 600 "$SCRIPT_DIR/cloudflare.env" /etc/clusteros/cloudflare.env
...
if [ -f "$SCRIPT_DIR/99-clusteros.yaml" ]; then
    ...
    warn "99-clusteros.yaml not in bundle — netplan not updated"
fi
...
# linux-modules-extra-$(uname -r) — provides WiFi driver modules (iwlwifi, ath9k,
# rtw88, etc.) split out of the base kernel package. The image build installs this
# against the Packer build host's kernel for verification, which can report
# "available" even when the install into the target image's /lib/modules/<kernel>
# silently failed — leaving the booted kernel with no WiFi driver at all.
dpkg -s "linux-modules-extra-$(uname -r)" &>/dev/null 2>&1 || PKGS="$PKGS linux-modules-extra-$(uname -r)"
...
# DKMS — needed to build out-of-tree WiFi drivers (RTL8852BU for UGREEN AX900 CM762)
dpkg -s dkms             &>/dev/null 2>&1 || PKGS="$PKGS dkms"
...
# ── RTL8852BU WiFi driver (UGREEN AX900 CM762) ────────────────────────────────
# This USB WiFi 6 adapter uses the Realtek RTL8852BU chipset which is not in the
# mainline kernel. Install via DKMS so it survives kernel updates.
_rtl8852bu_present() {
    modinfo 8852bu &>/dev/null || \
        find /lib/modules/"$(uname -r)" -name '8852bu.ko*' 2>/dev/null | grep -q .
}

if _rtl8852bu_present; then
    ok "RTL8852BU driver already installed"
else
    warn "RTL8852BU driver missing — installing for UGREEN AX900 CM762..."
    apt-get -o Acquire::ForceIPv4=true install -y -qq \
        build-essential "linux-headers-$(uname -r)" 2>/dev/null || \
        apt-get -o Acquire::ForceIPv4=true install -y -qq \
        build-essential linux-headers-generic 2>/dev/null || true
    ...
    if [ ! -d "$RTL_SRC" ] || [ ! -f "$RTL_SRC/dkms.conf" ]; then
        rm -rf "$RTL_SRC" /tmp/rtl8852bu-20250826-main
    ...
    if [ -f "$RTL_SRC/dkms.conf" ]; then
        ...
        # builds (8852bu, per BUILT_MODULE_NAME in dkms.conf) — modprobe and
        # modules-load.d need the module name, not the package name.
        ...
        dkms build "${RTL_NAME}/${RTL_VER}" -k "$(uname -r)" 2>/dev/null && \
        dkms install "${RTL_NAME}/${RTL_VER}" -k "$(uname -r)" --force 2>/dev/null && \
        modprobe "${RTL_MODULE}" 2>/dev/null || true
        if _rtl8852bu_present; then
            echo "${RTL_MODULE}" > /etc/modules-load.d/clusteros-rtl8852bu.conf
            ok "RTL8852BU driver installed and loaded"
        else
            warn "RTL8852BU build failed — check: dkms status; linux-headers-$(uname -r)"
        fi
    else
        warn "RTL8852BU source not available — check internet and retry: sudo bash apply-patch.sh"
    fi
fi
...
# Lab mode: this is a private, ringfenced cluster (no untrusted tenants, no
# public exposure). AppArmor profile denials and ufw/iptables default-DROP
# policies are a recurring source of debugging noise — e.g. blocked
# netlink/device operations during WiFi interface creation, or dropped DHCP
# replies on a freshly-created interface. Roll both back fully rather than
# chase individual denials/drops.

# AppArmor: stop the service so profiles aren't (re)loaded at boot, unload
# every profile currently loaded into the running kernel, and disable the
# LSM via the kernel cmdline so it stays off across reboots too.
if systemctl is-enabled apparmor &>/dev/null || systemctl is-active apparmor &>/dev/null; then
...
if [ -f /sys/kernel/security/apparmor/profiles ]; then
    while IFS= read -r line; do
        name="${line%% (*}"
        [ -n "$name" ] && echo -n "$name" > /sys/kernel/security/apparmor/.remove 2>/dev/null
    done < /sys/kernel/security/apparmor/profiles
    ok "AppArmor stopped and all loaded profiles unloaded"
fi
if [ -f /etc/default/grub ] && ! grep -q 'apparmor=0' /etc/default/grub; then
    sed -i 's/^\(GRUB_CMDLINE_LINUX_DEFAULT="[^"]*\)"/\1 apparmor=0"/' /etc/default/grub
    update-grub &>/dev/null 2>&1 || true
    ok "apparmor=0 added to kernel cmdline (takes effect after reboot)"
fi

# ufw: `disable` (not just stopping/disabling the service, as done later in
# this script) removes ufw's own iptables rules and resets its chain
# policies — the service-level disable only stops it reapplying them at boot.
if command -v ufw &>/dev/null; then
    ufw disable &>/dev/null 2>&1 || true
    ok "ufw disabled (rules removed, policies reset)"
fi

# Belt-and-suspenders: default-ACCEPT on all base chains regardless of what
# ufw/cloud-init/snap left behind. The Tailscale-only trust model for cluster
# ports is enforced by node-agent's setupFirewallRules(), not by blanket
# default-DROP chains.
...
ok "iptables/ip6tables default chain policies set to ACCEPT"
...
# NOTE: K3s API (6443), SLURM (6817/6818/6819), Kubelet (10250), and etcd
# (2379:2380) are intentionally NOT opened on the LAN here — they arrive
# exclusively via the Tailscale overlay (see setupFirewallRules() in
# daemon.go). Opening them on the LAN would contradict the Tailscale-only
# trust model and add attack surface with no benefit.
...
# ── WiFi setup ───────────────────────────────────────────────────────────────────
# Install WiFi credentials and ensure cluster-wifi.service is enabled.
# cluster-wifi-setup writes /etc/netplan/80-cluster-wifi.yaml with the detected
# interface name (networkd requires exact names, not match: patterns), then calls
# netplan apply and waits for an IP.

# Install WiFi credentials from bundle if present
if [ -f "$SCRIPT_DIR/wifi.env" ]; then
    mkdir -p /etc/clusteros
    install -m 600 "$SCRIPT_DIR/wifi.env" /etc/clusteros/wifi.env
    ok "WiFi credentials installed → /etc/clusteros/wifi.env"
fi

# Always (re)install cluster-wifi-setup from the bundle. A "missing binary"
# guard here would mean fixes to this script never reach nodes that were
# already patched — apply-patch.sh must keep it in sync on every run.
if [ -f "$SCRIPT_DIR/cluster-wifi-setup" ]; then
    install -m 755 "$SCRIPT_DIR/cluster-wifi-setup" /usr/local/bin/cluster-wifi-setup
    ok "cluster-wifi-setup installed → /usr/local/bin/cluster-wifi-setup"
fi

# Install and enable cluster-wifi.service if not already present
if [ -f "$SCRIPT_DIR/cluster-wifi.service" ] && [ ! -f /etc/systemd/system/cluster-wifi.service ]; then
    cp "$SCRIPT_DIR/cluster-wifi.service" /etc/systemd/system/cluster-wifi.service
    systemctl daemon-reload 2>/dev/null || true
    ok "cluster-wifi.service installed from bundle"
fi
if [ -f /etc/systemd/system/cluster-wifi.service ]; then
    systemctl enable cluster-wifi.service 2>/dev/null && \
        ok "cluster-wifi.service enabled (WiFi auto-connect on every boot)" || true
fi

# Run cluster-wifi-setup now if WiFi hardware is present and not yet connected.
# This brings up WiFi during apply-patch.sh so subsequent apt/tailscale steps have network.
if command -v cluster-wifi-setup &>/dev/null; then
    _wifi_up=false
    for _wlan_dir in /sys/class/net/*/wireless /sys/class/net/wl*; do
        [ -d "$_wlan_dir" ] || continue
        _wlan_if=$(basename "${_wlan_dir%%/wireless}")
        ip addr show "$_wlan_if" 2>/dev/null | grep -q 'inet ' && { _wifi_up=true; break; }
    done
    if [ "$_wifi_up" = false ] && [ -d /sys/class/ieee80211 ] && \
       [ -n "$(ls -A /sys/class/ieee80211 2>/dev/null)" ]; then
        ok "Running cluster-wifi-setup to connect WiFi before apt steps..."
        cluster-wifi-setup || true
    fi
fi

# Disable WiFi power-save so the NIC stays associated when Ethernet is also active.
for _wlan_dir in /sys/class/net/*/wireless; do
    [ -d "$_wlan_dir" ] || continue
    _wlan_iface=$(basename "$(dirname "$_wlan_dir")")
    iw dev "$_wlan_iface" set power_save off 2>/dev/null && \
        ok "WiFi power-save disabled on $_wlan_iface" || true
done
