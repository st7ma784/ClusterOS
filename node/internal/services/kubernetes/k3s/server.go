...
func (ks *K3sServer) DeployClusterServices(mungeKey []byte) {
	...
	// Cloudflare Tunnel — optional, activated by /etc/clusteros/cloudflare.env.
	// Deploys cloudflared as a k8s Deployment and adds host-based Ingress rules
	// so *.rmatool.org.uk routes through ingress-nginx to each service.
	if cfDomain, cfToken := readCloudflareEnv(); cfToken != "" {
		go ks.deployCloudflaredTunnel(cfDomain, cfToken)
	}
...
// readCloudflareEnv reads /etc/clusteros/cloudflare.env and returns the domain and tunnel token.
// Returns empty strings if the file is absent or the keys are not set.
func readCloudflareEnv() (domain, token string) {
...
// deployCloudflaredTunnel deploys cloudflared inside k3s so that *.domain traffic is routed
// through Cloudflare's edge to the cluster's ingress-nginx without any public inbound ports.
// It also creates host-based Ingress rules for each cluster service so ingress-nginx can
// route by the custom domain hostname (longhorn.domain, rancher.domain, etc.).
func (ks *K3sServer) deployCloudflaredTunnel(domain, token string) {
	ks.Logger().Infof("Deploying Cloudflare Tunnel for domain %s", domain)
...
`
	...
	if err := apply(nsYAML); err != nil {
		ks.Logger().Warnf("cloudflared namespace: %v", err)
		return
	}

	// Secret holding the tunnel token — create or overwrite so re-deploys pick up a new token.
	secretCmd := exec.Command("k3s", "kubectl", "-n", "cloudflared", "create", "secret", "generic",
		"cloudflared-token",
		"--from-literal=token="+token,
		"--dry-run=client", "-o", "yaml")
	secretYAML, err := secretCmd.Output()
	if err != nil {
		ks.Logger().Warnf("cloudflared token secret render: %v", err)
		return
	}
	applyCmd := exec.Command("k3s", "kubectl", "apply", "-f", "-")
	applyCmd.Stdin = bytes.NewReader(secretYAML)
	if out, err := applyCmd.CombinedOutput(); err != nil {
		ks.Logger().Warnf("cloudflared token secret apply: %v: %s", err, out)
		return
	}

	// 2-replica Deployment — cloudflared reconnects outbound to Cloudflare edge.
	// Routes all *.domain and bare domain traffic to ingress-nginx ClusterIP on port 80.
	ingressNginxSvc := "http://ingress-nginx-controller.ingress-nginx.svc.cluster.local"
	tunnelConfig := fmt.Sprintf(`tunnel: clusteros
ingress:
  - hostname: "*.%s"
    service: %s
  - hostname: "%s"
...
`, domain, ingressNginxSvc, domain, ingressNginxSvc)
...
data:
  config.yaml: |
%s`, indent(tunnelConfig, "    "))
	if err := apply(configMapYAML); err != nil {
		ks.Logger().Warnf("cloudflared configmap: %v", err)
		return
	}
    ...
    spec:
      ...
      - name: config
        configMap:
          name: cloudflared-config
`
	if err := apply(deploymentYAML); err != nil {
		ks.Logger().Warnf("cloudflared deployment: %v", err)
		return
	}

	ks.Logger().Infof("cloudflared deployed — applying host-based Ingress rules for %s", domain)
	ks.reconcileCloudflareIngresses(domain)
	ks.Logger().Infof("Cloudflare Tunnel active: *.%s → ingress-nginx", domain)
}

// reconcileCloudflareIngresses adds host-based Ingress rules alongside existing path rules so
// that each service is reachable at <service>.<domain> via the Cloudflare Tunnel.
// Existing path-based rules (used for direct IP/Tailscale access) are not modified.
func (ks *K3sServer) reconcileCloudflareIngresses(domain string) {
	apply := func(yaml string) {
		cmd := exec.Command("k3s", "kubectl", "apply", "-f", "-")
		cmd.Stdin = strings.NewReader(yaml)
		if out, err := cmd.CombinedOutput(); err != nil {
			ks.Logger().Warnf("cloudflare ingress: %v: %s", err, out)
		}
	}

	// Longhorn — serves at / when accessed via its own hostname (no path rewrite needed).
...
`, domain))
	...
	// Rancher — HTTPS backend on port 443.
	apply(fmt.Sprintf(`apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
  name: rancher-cf
...
`, domain))
	...
	// JupyterHub — standard HTTP on port 80.
	apply(fmt.Sprintf(`apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
  name: jupyter-cf
...
// indent prepends each non-empty line of s with prefix.
