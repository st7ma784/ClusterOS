...
#   SSH_PASS=<password>         — set explicitly for one-time bootstrap before keys are deployed
#                                 e.g. make setup-ssh-keys SSH_PASS=<temporary-password>
...
# SSH_PASS is intentionally unset by default — nodes use key auth only.

# Internal: key-only by default; sshpass wrapper added only when SSH_PASS is explicitly set.
_SSH_AUTH := $(if $(SSH_PASS),sshpass -p '$(SSH_PASS)') ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 $(if $(SSH_KEY),-i $(SSH_KEY)) $(if $(SSH_PASS),,-o PasswordAuthentication=no)
_SCP_AUTH := $(if $(SSH_PASS),sshpass -p '$(SSH_PASS)') scp -o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 $(if $(SSH_KEY),-i $(SSH_KEY))
...
deploy: patch
	...
	@echo "SSH user: $(SSH_USER)  key: $(SSH_KEY)  $(if $(SSH_PASS),password: set (bootstrap mode),password: disabled (key-only))"
